using MarsRoverTest.Interfaces;
using System;
            ...
            foreach (var move in moves)
            {
                switch (move)
                {
                    ...
                    case 'R':
                        ...
                        break;
                    default:
                        Console.WriteLine(String.Format("Invalid Character {0}", move));
                        break;
                }

                if (this.X < 0 || this.X > maxPoints[0] || this.Y < 0 || this.Y > maxPoints[1])
                {
                    throw new Exception(String.Format("Oops! Position can not be beyond bounderies (0 , 0) and ({0} , {1})", maxPoints[0], maxPoints[1]));
                }
            }
        }
    }
